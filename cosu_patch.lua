...
function update(event, ...)
    ...
    elseif event == "now" then
        ...
        if gitAPI.getResponseCode() == 200 then
            ...
            elseif remos then
                file = fs.open(fs.combine(fs.getDir(remos.getRunningProgram()), "cosu.lua"), "w")
            else
                    ...
                    ["func"] = function(pop)
                        ...
                        elseif remos then
                            if fs.isDir(link) then
                                remos.addAppFileEnv(shell.resolveProgram("shell"), _ENV)
                            else
                                remos.addAppFileEnv(remos.getRunningProgram(), _ENV, link)
                            end
                            tActiveKeys["CTRL"] = false
...
function file(event, ...)
    ...
    elseif event == "saved" then
        ...
        elseif remos then
            remos.setTitle("CONSULT - " .. sPathName)
        elseif multishell then
    ...
    elseif event == "create" then
            ...
            table.insert(tPopup, 1, {
                ...
                ["button"] = {
                    ...
                    },
                    {
                        ...
                        ["status"] = false,
                        ["func"] = function()
                            local tmp = tPopup[1].textBox[1].input
                            file("close")
                            file("create", "save", tmp)
                        end
                    }
                }
            })
        elseif tArgs[1] == "new" then
            if bLvlOS then
                ...
                category.reset()
            elseif multishell then
                local tabId = multishell.launch(_ENV, remos.getRunningProgram())
    ...
    elseif event == "execute" then
        ...
        if remos then
            sDir = '/' .. fs.getDir(remos.getRunningProgram()) .. '/'
        end
        ...
        elseif remos then
            local pid = remos.addAppFileEnv(sDir .. ".tmp" .. sCurID, _ENV, ...)
            remos.setTitle("cosu-[run[" .. pid .. "]")
        elseif multishell then
...
function options(event, ...)
    local tSubArgs = { ... }
    local sDir = fs.getDir(remos and remos.getRunningProgram() or shell.getRunningProgram()) .. '/'
    if event == "close" then
        for index, pop in pairs(tPopup) do
    ...
    elseif event == "create" then
        ...
        elseif remos then
            local pid = remos.addAppFileEnv(remos.getRunningProgram(), _ENV, sDir .. ".cosu.conf")
            remos.setTitle("CONSULT - [options]", pid)
        elseif multishell then
...
local tInfobar = {
    ...
    function()
        return cToStr(colorMatch.cAccentText) ..
    ...
    function()
        ...
        return cToStr(colorMatch.special) ..
            "[" .. cToStr(colorMatch.text) .. size ..
            cToStr(colorMatch.cAccentText) .. sizeType .. cToStr(colorMatch.special) .. "]"
    end,
}
}
...
function init()
    ...
    elseif remos then
        remos.setTitle("CONSULT - " .. fs.getName(sPath))
    elseif multishell then
